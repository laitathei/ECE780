...
class manipulator_control:
    def __init__(self, L, ML, IL, D, qd, qd_dot, q, q_dot, q_dot_dot, x, x_dot, x_dot_dot, xd, xd_dot, xd_dot_dot):
        ...
        self.x_dot_dot = x_dot_dot # end-effector acceleration
        self.xd = xd # desired end-effector position
        self.xd_dot = xd_dot # desired end-effector velocity
        self.xd_dot_dot = xd_dot_dot # desired end-effector acceleration
        self.g = 9.81 # gravity
        self.Fc11 = 0
        self.Fc22 = 0
        self.Fv11 = 0.1
    ...
    def jacobian(self, q):
        """
        Calculate the jacobian matrix of the manipulator based on the joint angle
    ...
    def derivative_jacobian(self, q, q_dot):
        """
        Calculate the derivative jacobian matrix of the manipulator based on the joint angle and velocity
        ...
        L1, L2 = self.L[0][0], self.L[1][0] # length of each link
        q1, q2 = q[0][0], q[1][0] # joint angle
        q_dot_1, q_dot_2 = q_dot[0][0], q_dot[1][0] # joint velocity
        j_dot_11 = -L1*np.cos(q1)*q_dot_1-L2*np.cos(q1+q2)*(q_dot_1+q_dot_2)
        j_dot_12 = -L2*np.cos(q1+q2)*(q_dot_1+q_dot_2)
        j_dot_21 = -L1*np.sin(q1)*q_dot_1-L2*np.sin(q1+q2)*(q_dot_1+q_dot_2)
        j_dot_22 = -L2*np.sin(q1+q2)*(q_dot_1+q_dot_2)
        J_dot = np.array([[j_dot_11, j_dot_12],[j_dot_21, j_dot_22]])
        return J_dot
    
    def main(self, M_d, Kd, Kp, elastic):
        """
        ...
        :param float M: mass matrix parameter
        :param bool elastic: elastic wall exist or not
        """
        self.M_d = np.diag(np.array([M_d, M_d]))
        self.Kd = np.diag(np.array([Kd, Kd]))
        self.Kp = np.diag(np.array([Kp, Kp]))
        pos_x_list = []
        pos_y_list = []
        q1_list = []
        q2_list = []
        ...
        with Bar('Processing... ', max=int(self.T/self.dt)) as bar:
            for i in range (int(self.T/self.dt)):
                ...
                J_dot = self.derivative_jacobian(self.q, self.q_dot)
                ...
                u = np.linalg.inv(J) @ np.linalg.inv(self.M_d) @ (self.M_d @ self.xd_dot_dot + self.Kd @ (self.xd_dot-self.x_dot) + self.Kp @ (self.xd-self.x) - self.M_d @ J_dot @ self.q_dot - Fe)

                # Calculate the torque
                tau = D @ u + C @ self.q_dot + G + J.T @ Fe
        ...
        self.plot_force(force_list, iternation_list)
    ...
    def plot_force(self, force_list, iternation_list):
        # plot magnitude of the force exerted by the wall on the robot end effector
        fig, axs = plt.subplots(1, 1, figsize=(12, 6))
        axs.plot(iternation_list, force_list, label="wall force") # joint torque 1
        axs.set_xlabel('Time [{}s]'.format(self.dt))
        axs.set_ylabel('Force [N]')
        axs.legend()

        plt.tight_layout()
        plt.savefig('script\\assignment\\assignment_2_picture\\{}_joint_force.png'.format(self.text))
        plt.show()
...
q = np.array([[np.pi/2], [-np.pi/2]], dtype=float) # initial joint anlge
...
x_dot_dot = np.array([[0], [0]], dtype=float) # initial end-effector acceleration
...
xd_dot_dot = np.array([[0], [0]], dtype=float) # desired end-effector acceleration
sim = manipulator_control(L, ML, IL, D, qd, qd_dot, q, q_dot, q_dot_dot, x, x_dot, x_dot_dot, xd, xd_dot, xd_dot_dot)
M_d = 1.0
Kd = 10 # Kd
Kp = 25 # Kp
elastic = True # False for Q3i, True for Q3ii
sim.main(M_d, Kd, Kp, elastic)